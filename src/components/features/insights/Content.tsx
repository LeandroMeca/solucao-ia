...
function SectionTitle({ children }: PropsWithChildren) {
  return (
    <h3 className="loading-relaxed mb-1.5 mt-5 text-sm font-semibold text-foreground">
      {children}
    </h3>
  );
}
...
const statusStyles = {
  ...
  needs_adjustment: {
    ...
    className:
      'bg-green-100 text-amber-700 dark:bg-green-900/30 dark:text-green-400',
  },
  unfeasible: {
    label: 'Meta inviável no prazo',
    className:
      'bg-green-100 text-red-700 dark:bg-green-900/30 dark:text-green-400',
  },
};
    ...
    <div className="lg:scrollbar-thin lg:max-h-93 lg:overflow-y-auto lg:pr-2 lg:[scrollbar-color:var(--border)_transparent]">
      <section className="flex flex-col gap-2">
        <div className="flex flex-col items-start gap-2 sm:flex-row">
          ...
          {status && (
            <span
              className={`w-fit rounded-full px-2.5 text-xs font-semibold ${status.className}`}
            >
              {status.label}
            </span>
          )}
      ...
      <section>
        <SectionTitle>Sugestões de Imvestimento</SectionTitle>
